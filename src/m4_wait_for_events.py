...
def sum_until_prime_input():
    ...
    total = 0
    while True:
        x= int(input("Enter an integer that is at least 2: "))
        total += x
        if is_prime(x):
            break
    print(total)
...
3      -- prime_gap(8) returns 89, because the next prime after 89 is 97,
    ...
    smallest = 2
    while True:
        if next_prime(smallest+1) - smallest >= m :
            break
        smallest = next_prime(smallest+1)
    return smallest
